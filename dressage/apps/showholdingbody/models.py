...
class ShowHoldingBodyStatus(models.TextChoices):
    """Status choices for Show Holding Body"""
...
class ShowHoldingBody(models.Model):
    """
    Model representing a Show Holding Body organization.
    
    A Show Holding Body is an organization that can host and manage
    ...
    status = models.CharField(
        max_length=20,
        choices=ShowHoldingBodyStatus.choices,
        default=ShowHoldingBodyStatus.ACTIVE
    )
    ...
    class Meta:
        verbose_name = "Show Holding Body"
    ...
    @property
    def is_active(self):
        """Check if the show holding body is active"""
        return self.status == ShowHoldingBodyStatus.ACTIVE
